    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.links.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from_node') ? 'is-invalid' : '' }}" name="from_node_id" id="from_node_id">
                    @foreach($from_nodes as $id => $entry)
                        <option value="{{ $id }}" {{ old('from_node_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('to_node') ? 'is-invalid' : '' }}" name="to_node_id" id="to_node_id">
                    @foreach($to_nodes as $id => $entry)
                        <option value="{{ $id }}" {{ old('to_node_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('connection_type') ? 'is-invalid' : '' }}" type="text" name="connection_type" id="connection_type" value="{{ old('connection_type', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_created') ? 'is-invalid' : '' }}" name="user_created_id" id="user_created_id">
                    @foreach($user_createds as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_created_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('affect_node') ? 'is-invalid' : '' }}" name="affect_node_id" id="affect_node_id">
                    @foreach($affect_nodes as $id => $entry)
                        <option value="{{ $id }}" {{ old('affect_node_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('affected_svg_state') ? 'is-invalid' : '' }}" type="text" name="affected_svg_state" id="affected_svg_state" value="{{ old('affected_svg_state', '') }}">
