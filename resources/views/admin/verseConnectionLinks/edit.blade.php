...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.verseConnectionLink.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.verse-connection-links.update", [$verseConnectionLink->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('link') ? 'is-invalid' : '' }}" name="link_id" id="link_id">
                    @foreach($links as $id => $entry)
                        <option value="{{ $id }}" {{ (old('link_id') ? old('link_id') : $verseConnectionLink->link->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('link'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('verse') ? 'is-invalid' : '' }}" type="text" name="verse" id="verse" value="{{ old('verse', $verseConnectionLink->verse) }}">
                @if($errors->has('verse'))
                    <div class="invalid-feedback">
                        {{ $errors->first('verse') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $verseConnectionLink->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
