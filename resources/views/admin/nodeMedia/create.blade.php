...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.nodeMedium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.node-media.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nodes_related_tos') ? 'is-invalid' : '' }}" name="nodes_related_tos[]" id="nodes_related_tos" multiple>
                    @foreach($nodes_related_tos as $id => $nodes_related_to)
                        <option value="{{ $id }}" {{ in_array($id, old('nodes_related_tos', [])) ? 'selected' : '' }}>{{ $nodes_related_to }}</option>
                    @endforeach
                </select>
