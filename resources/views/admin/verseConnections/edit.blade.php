...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.verseConnection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.verse-connections.update", [$verseConnection->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="verses">{{ trans('cruds.verseConnection.fields.verses') }}</label>
                <input class="form-control {{ $errors->has('verses') ? 'is-invalid' : '' }}" type="text" name="verses" id="verses" value="{{ old('verses', $verseConnection->verses) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nodes') ? 'is-invalid' : '' }}" name="nodes[]" id="nodes" multiple>
                    @foreach($nodes as $id => $node)
                        <option value="{{ $id }}" {{ (in_array($id, old('nodes', [])) || $verseConnection->nodes->contains($id)) ? 'selected' : '' }}>{{ $node }}</option>
                    @endforeach
                </select>
                @if($errors->has('nodes'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $verseConnection->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
